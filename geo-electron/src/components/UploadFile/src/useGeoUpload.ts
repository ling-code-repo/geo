import * as FileApi from '@/api/geo/file'
import { getFilePresignedUrl } from '@/api/infra/file'
import { UPLOAD_TYPE } from '@/components/UploadFile/src/useUpload'
import {
  UploadRawFile,
...
export const getUploadUrl = (): string => {
  return import.meta.env.VITE_BASE_URL + import.meta.env.VITE_API_URL + '/geo/file/upload'
}

export const useUpload = (category?: string,directory?: string) => {
  // 后端上传地址
  ...
  const httpRequest = async (options: UploadRequestOptions) => {
    ...
    if (isClientUpload) {
      ...
      return axios
        ...
        .then(() => {
          // 1.4. 记录文件信息到后端（异步）
          createFile(presignedInfo, options.file, fileName, category)
      ...
      return new Promise((resolve, reject) => {
        FileApi.updateFile({ file: options.file, directory, category }, uploadProgressHandler)
          .then((res) => {
            if (res.code === 0) {
              resolve(res)
            } else {
