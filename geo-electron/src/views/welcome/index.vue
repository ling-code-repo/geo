...
import { Document, EditPen, Key, User } from "@element-plus/icons-vue";
import { StatisticsApi } from "@/api/geo/statistics";
import { setupInitListener } from "@/composables/listenter";
...
const initMetricCards = async () => {
  const data = await StatisticsApi.getMetric();
  const dataMap = new Map();
  data.forEach((d) => {
    dataMap.set(d.name, d.value);
  });
  metricCards.value.forEach((item) => {
    item.value = dataMap.get(item.title) || 0;
  });
};

// 初始化AI创作图表
const initAiCreationChart = async () => {
  try {
    if (!aiCreationChartRef.value) return;
    const data = await StatisticsApi.getArticle();
...
// 初始化投喂数据图表
const initFeedDataChart = async () => {
  try {
    if (!feedDataChartRef.value) return;
    const data = await StatisticsApi.getRecord();
...
// const {globalConnected} = useGlobalSocket ()
// const {localConnected} = useLocalSocket ()
      ...
      setupInitListener()
        setTimeout(() => {
          window.dispatchEvent(
            new Event("init-event"),
          );
        }, 100);


      await initMetricCards();
      await initAiCreationChart();
      // initCollectionRatioChart();
      await initFeedDataChart();
