<template>
  ...
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    ...
    >
      ...
      <el-form-item label="封面图" prop="pictureIdList">
      ...
      <el-form-item label="文章内容" prop="content">
        <span style="color: red">抖音、小红书类文字建议控制在1000字之内，会自动清除展示排版</span>
        <Editor v-model="formData.content" placeholder="请输入文章内容" height="300px" />
      </el-form-item>
    </el-form>
    <template #footer>
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      ...
      formData.value['content'] = markdownToHtml(formData.value['contentMarkdown'])
...
const submitForm = async () => {
  ...
  try {
    ...
    data['contentMarkdown'] = htmlToMarkdown(data['content'])
    data.pictureIds = formData.value.pictureIdList?.join(",")
