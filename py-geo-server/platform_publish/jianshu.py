...
steps = [
        ...
        {
            "type": "navigate",
            "selector": "https://www.jianshu.com/writer#/",
            "timeout": 3000,
            "descript": "正在打开主页"
        },
        ...
        # 3. 如果有弹窗（比如登录/通知弹窗），点击关闭
        {
            "type": "click",
            "selector": '//span[text()="新建文集"]',
            "is_exist": 1,
            "descript": "新建文集",
            "timeout": 3000
        },

        # 4. 输入文章标题（变量 title 将在外部替换）
        {
            "type": "fill",
            "selector": "input[placeholder='请输入文集名...']",
            "descript": "输入文集",
            "value": "article_set"
        },
    ...
    {
        "type": "fill",
        "selector": "div[class*='ant-col'] input",
        "descript": "输入标题",
        "value": "title",
"nth": 1,
    },
        ...
        {
            "type": "click",
            "selector": '//a[text()="发布文章"]',
            "descript": "点击发布"
        }
    ]

class Jianshu(Publish):
        ...
        for step in steps:
            ...
            if _type in ['click', 'fill', 'input_files', 'hover']:
                try:
                    ...
                    if _type == 'fill':
                        if descript == '输入文集':
                            await locator.fill(f"文集 {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
                        else:
                            if descript == 'title':
                                await locator.clear()
            ...
            if _type == 'press':
                if value == 'content':
                    editor = page.locator('div[contenteditable="true"].kalamu-area')
                    await editor.focus()
                    # locator = page.locator(selector)
                    # if nth != '':
                    #     locator = locator.nth(nth)
